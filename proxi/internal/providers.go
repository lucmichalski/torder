...
func githubOpsxcq(ctx context.Context) Proxies {
	defer ctx.Done()
	start := time.Now()
	var (
		foundProxies Proxies
		mu           sync.Mutex
		source       = "github.com/opsxcq"
		baseURL      = "https://github.com/opsxcq/proxy-list/blob/master/list.txt"
	)
	done := make(chan bool)
...
func githubHookzof(ctx context.Context) Proxies {
	defer ctx.Done()
	start := time.Now()
	var (
		foundProxies Proxies
		mu           sync.Mutex
		source       = "github.com/hookzof"
		baseURL      = "https://github.com/hookzof/socks5_list/blob/master/proxy.txt"
	)
	done := make(chan bool)
...
func githubA2u(ctx context.Context) Proxies {
	defer ctx.Done()
	start := time.Now()
	var (
		foundProxies Proxies
		mu           sync.Mutex
		source       = "github.com/a2u"
		baseURL      = "https://github.com/a2u/free-proxy-list/blob/master/free-proxy-list.txt"
	)
	done := make(chan bool)
